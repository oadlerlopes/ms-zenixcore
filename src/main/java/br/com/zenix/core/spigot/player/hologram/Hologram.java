...
public class Hologram {
	...
	public Hologram(String text, Location location, boolean register) {
		...
		TIntObjectMap<WatchableObject> dataValues = (TIntObjectMap<WatchableObject>) Utils.getValue("dataValues", dataWatcher);
		...
		for (int i = 0; i < 100; i++) {
			...
			int index = (int) Utils.getValue("b", current);

			if (index == 2) {

				Utils.setValue("c", current, getText());
			} else if (index == 3) {

				Utils.setValue("c", current, getText().isEmpty() ? (byte) 0 : (byte) 1);
			} else {
		...
		Utils.setValue("b", spawnArmorStand1_8, 30);
	...
	public void lock(final Player player, long ticks) {
		...
		}.runTaskLater(Core.getPlugin(Core.class), ticks);
	}
