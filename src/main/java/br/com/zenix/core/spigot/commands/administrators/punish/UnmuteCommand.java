...
public class UnmuteCommand extends BukkitCommand {
	...
	public boolean execute(CommandSender commandSender, String label, String[] args) {
		...
		if (args.length < 1) {
			commandSender.sendMessage("§aUse: §f/unmute <player/uniqueid>");
		} else {
		...
		public void run() {
			...
			if (getCoreManager().getPunishManager().canPunish(args[0])) {
				...
				if (!getCoreManager().getPunishManager().hasPunishActive(id, type)) {
					commandSender.sendMessage("§cO player não tem nenhum mute para ser revogado.");
					return;
				}
				...
				PunishRecord record = records.stream().filter(rec -> rec.isActive() && rec.getType() == type)
				...
				if (getCoreManager().getPunishManager().unPunishPlayer(id, record.getId())) {
					commandSender.sendMessage("§aVocê desmutou o jogador §f" + args[0]);
				} else {
					commandSender.sendMessage("§cDeu merda!");
					return;
				}
