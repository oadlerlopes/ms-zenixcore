package br.com.zenix.core.spigot.anticheat.util;
...
public class UtilTime {
	...
	public static String convertString(long time, int trim, TimeUnit type) {
		...
		if (type == TimeUnit.DAYS) {
			return UtilMath.trim(trim, time / 8.64E7D) + " dias";
		}
		if (type == TimeUnit.HOURS) {
			return UtilMath.trim(trim, time / 3600000.0D) + " horas";
		}
		if (type == TimeUnit.MINUTES) {
			return UtilMath.trim(trim, time / 60000.0D) + " minutos";
		}
		if (type == TimeUnit.SECONDS) {
			return UtilMath.trim(trim, time / 1000.0D) + " segundos";
		}
		return UtilMath.trim(trim, time) + " miliseconds";
	}
