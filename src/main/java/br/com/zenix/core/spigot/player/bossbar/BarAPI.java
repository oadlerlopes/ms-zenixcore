package br.com.zenix.core.spigot.player.bossbar;
...
import br.com.zenix.core.spigot.player.bossbar.dragon.FakeDragon;
import br.com.zenix.core.spigot.player.bossbar.util.Util;
...
public class BarAPI extends BukkitListener {
	...
	public static void clear() {
		for (Player player : Core.getCoreManager().getPlugin().getServer().getOnlinePlayers()) {
			quit(player);
		}
	...
	private void handleTeleport(final Player player, final Location loc) {
		...
		Bukkit.getScheduler().runTaskLater(Core.getCoreManager().getPlugin(), new Runnable() {
	...
	@EventHandler
	public void onTick(ServerTimeEvent event) {
		if (players.isEmpty())
			return;
		--ticks;
		if (ticks <= 0) {
			...
			for (UUID uuid : players.keySet()) {
				...
				Util.sendPacket(player, players.get(uuid).getTeleportPacket(BarAPI.getDragonLocation(player)));
			}
		}
	}
	...
	public static void setMessage(final Player player, String message, int seconds) {
		...
		timers.put(player.getUniqueId(), Bukkit.getScheduler().runTaskLater(Core.getCoreManager().getPlugin(), new Runnable() {
			@Override
			public void run() {
				removeBar(player);
				cancelTimer(player);
